...
function validateSupabaseEnvOrExit() {
  ...
  if (!url || !key) {
    console.error(
      "\n❌ [SUPABASE] SUPABASE_URL ve SUPABASE_SERVICE_ROLE_KEY zorunludur.\n" +
        "   Proje kökünde .env oluşturun veya export edin; örnek: .env.example\n"
    );
    process.exit(1);
  }
  ...
  if (looksPlaceholder) {
    ...
    console.error(
      "   Örnek: https://abcdefghijklmnop.supabase.co (kendi proje ref’iniz)\n"
    );
    process.exit(1);
  }
  ...
  if (key.length < 32 || key === "dummy") {
    console.error(
      "\n❌ [SUPABASE] SUPABASE_SERVICE_ROLE_KEY geçersiz veya placeholder (ör. dummy).\n" +
  ...
  try {
    ...
    /* unreachable: URL already validated */
  }
}
