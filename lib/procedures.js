...
// Used by:
//   GET /api/procedures?lang=   (index.cjs — admin-settings loadProcedureDefs)
//   GET /api/doctor/procedures  (index.cjs)
//   getProcedureLabel          (routes/doctor/treatments.js, …)
//
// Admin ayarlar «Tedavi fiyat listesi»: prosedür satır etiketleri API’den seçilen dile göre gelir (en|tr|ru|ka).
...
/**
 * Full procedure catalog for GET /api/procedures and GET /api/doctor/procedures.
 * @param {string} [lang] query lang: en | tr | ru | ka
 */
function buildProceduresCatalogPayload(lang) {
  const l = normalizeCatalogLang(lang);
  const types = PROCEDURE_TYPES_RAW.map((p) => ({
    type: p.type,
    label: getProcedureLabel(p, l),
    category: p.category,
  }));
