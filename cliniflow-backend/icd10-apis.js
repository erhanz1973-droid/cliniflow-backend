...
module.exports = (app) => {
  ...
  app.get("/api/icd10/codes", async (req, res) => {
    try {
      ...
      // Select only the required fields and alias them for frontend
      let selectFields = [
        'code',
        'title_en',
        'title_tr',
      ...
      let query = supabase.from("icd10_codes").select(selectFields.join(", "));
      ...
      if (search) {
        // Search in all title fields and code
        query = query.or([
          'title_en.ilike.%' + search + '%',
          'title_tr.ilike.%' + search + '%',
          'title_ru.ilike.%' + search + '%',
          'title_ka.ilike.%' + search + '%',
          'code.ilike.%' + search + '%'
        ].join(","));
      }
      ...
      const mappedCodes = (codes || []).map(code => ({
        code: code.code,
        name_en: code.title_en,
        name_tr: code.title_tr,
        name_ru: code.title_ru,
        name_ka: code.title_ka,
        description_en: code.description_en,
        description_tr: code.description_tr,
        description_ru: code.description_ru,
        description_ka: code.description_ka
      }));
