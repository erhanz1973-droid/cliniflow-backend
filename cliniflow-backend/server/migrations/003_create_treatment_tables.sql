-- Patient Encounters (Muayene/Vakalar)
...
-- Encounter Diagnoses (ICD-10 Tanıları)
CREATE TABLE IF NOT EXISTS encounter_diagnoses (
    ...
    CONSTRAINT unique_primary_diagnosis UNIQUE (encounter_id, is_primary) 
...
CREATE TABLE IF NOT EXISTS treatment_plans (
    ...
    CONSTRAINT unique_active_plan_per_encounter UNIQUE (encounter_id, status)
        DEFERRABLE INITIALLY DEFERRED
);

-- Treatment Items (Diş İşlemleri)
...
-- Treatment Item Prices (Fiyatlandırma)
...
-- Treatment Activity Log (İşlem Geçmişi)
...
-- Check constraint: At least one primary diagnosis per encounter
CREATE OR REPLACE FUNCTION ensure_primary_diagnosis()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' AND NEW.is_primary = false THEN
        -- Check if this is the first diagnosis for this encounter
        IF (SELECT COUNT(*) FROM encounter_diagnoses WHERE encounter_id = NEW.encounter_id) = 0 THEN
            NEW.is_primary = true;
        END IF;
    END IF;
    RETURN NEW;
...
CREATE TRIGGER ensure_primary_diagnosis_trigger
    BEFORE INSERT ON encounter_diagnoses
    FOR EACH ROW
    EXECUTE FUNCTION ensure_primary_diagnosis();
