...
async function getPatientsByClinic(clinicId) {
  ...
  try {
    ...
    // Add name field mapping (canonical: patients.name)
    return (data || []).map(patient => {
      let display =
        String(patient.name || "").trim() ||
        String(patient.full_name || "").trim();
      if (!display && patient.first_name && patient.last_name) {
        display = `${patient.first_name} ${patient.last_name}`.trim();
      }
      const fallbackId = patient.patient_id || "";
      return {
        ...patient,
        fullName: display || fallbackId,
        name: display || fallbackId,
      };
    });
  } catch (error) {
