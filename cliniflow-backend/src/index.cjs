...
app.post("/api/patient/login", async (req, res) => {
  try {
    ...
    // Strip all non-digits then try multiple formats:
    // "05321234567" → "5321234567", "+905321234567" → "905321234567" → "5321234567"
    const digitsOnly = rawPhone.replace(/[^\d]/g, "");
    // Build candidate list: original trim, digits-only, strip leading country code 90
    const candidates = [
      rawPhone,
      digitsOnly,
      digitsOnly.startsWith("90") && digitsOnly.length === 12 ? digitsOnly.slice(2) : null,
      digitsOnly.startsWith("0") ? digitsOnly.slice(1) : null,
      "0" + digitsOnly,
    ].filter((v, i, a) => v && a.indexOf(v) === i); // unique, non-null

    // Hasta bul — any matching phone format
    ...
    const trimmedPhone = rawPhone; // kept for legacy log messages below

    if (dbError) {
      console.error("[PATIENT LOGIN] Database error:", dbError);
...
app.get("/health", (req, res) => {
  res.json({ ok: true, port: String(PORT), entry: "cliniflow-backend/src/index.cjs" });
});
